...
class ProxyTraversalTest(ZopeTestCase):
    ...
    def test_img_downloader_zope_file(self):
        # the img downloader must also work with OFS.Image.File objects
        proxy = self.proxy
        doc = proxy.getContent()
        f = open(os.path.join(TEST_DATA_PATH, 'logo_cps.png'))
        doc._setObject('fobj', File('fobj', 'myimg.png', f))
        # we assume that content-type is correct on the file object
        doc.fobj.content_type = 'image/png'
    ...
    def test_img_downloader_none(self):
        proxy = self.proxy
        doc = proxy.getContent()
        doc.fobj = None
        ...
        req = self.folder.REQUEST
        img_content = dl.index_html(req, req.RESPONSE)
        self.assertEquals(img_content, '')
        self.assertEquals(req.RESPONSE['content-length'], '0')
